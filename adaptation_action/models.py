...
class AdaptationAction(models.Model):
    ...
    fsm_state = FSMField(default='nuevo', protected=True, max_length=100)
    ...
    def can_submit(self):
        ...
        ## check current status is nuevo
        return self.fsm_state in ['nuevo','actualizando_por_solicitud_DCC']
  
    def can_evaluate_by_DCC(self):
        ## check current status is enviado
        return self.fsm_state == 'enviado'

    ## rechazado_por_DCC, cambios_solicitados_por_DCC, aceptado_por_DCC
    def can_rejected_by_DCC(self):
        ## check current status is enviado
        return self.fsm_state == 'en_evaluación_por_DCC'
    ...
    def can_update_by_DCC_request(self):
        ## check current status is enviado
        return self.fsm_state == 'cambios_solicitados_por_DCC'

    @transition(field='fsm_state', source=['nuevo', 'actualizando_por_solicitud_DCC'], target='enviado', conditions=[can_submit], on_error='enviado')
    def submit(self, user_approver):
        # nuevo --> enviado        
        # send email to user that enviado the action
        print(f'The adaptation action is transitioning from <{self.fsm_state}> to <enviado>')
        
        email_function = {
            'nuevo': self.email_service.notify_dcc_responsible_adaptation_action_submission
        }
    ...
    @transition(field='fsm_state', source='enviado', target='en_evaluación_por_DCC', conditions=[can_evaluate_by_DCC], on_error='enviado', permission='')
    def evaluate_by_DCC(self, user_approver):
        # enviado --> en_evaluación_por_DCC
        # send email to user that enviado the action
        print('The adaptation action is transitioning from <enviado> to <en_evaluación_por_DCC>')
    ...
    ## rechazado_por_DCC, cambios_solicitados_por_DCC, aceptado_por_DCC
    ##
    @transition(field='fsm_state', source='en_evaluación_por_DCC', target='rechazado_por_DCC', conditions=[can_rejected_by_DCC], on_error='en_evaluación_por_DCC', permission='')
    def evaluate_by_DCC_rejected(self, user_approver):
        # en_evaluación_por_DCC --> rechazado_por_DCC
        # send email to user that enviado the action
        print('The adaptation action is transitioning from <en_evaluación_por_DCC> to <rechazado_por_DCC>')
    ...
    @transition(field='fsm_state', source='en_evaluación_por_DCC', target='cambios_solicitados_por_DCC', conditions=[can_request_changes_by_DCC], on_error='en_evaluación_por_DCC', permission='')
    def evaluate_by_DCC_requested_changes(self, user_approver):
        # en_evaluación_por_DCC --> cambios_solicitados_por_DCC
        # send email to user that enviado the action
        print('The adaptation action is transitioning from <en_evaluación_por_DCC> to <cambios_solicitados_por_DCC>')
    ...
    @transition(field='fsm_state', source='en_evaluación_por_DCC', target='aceptado_por_DCC', conditions=[can_acception_by_DCC], on_error='en_evaluación_por_DCC', permission='')
    def evaluate_by_DCC_accepted(self, user_approver):
        # en_evaluación_por_DCC --> aceptado_por_DCC
        # send email to user that enviado the action
        print('The adaptation action is transitioning from <en_evaluación_por_DCC> to <aceptado_por_DCC>')
    ...
    @transition(field='fsm_state', source='rechazado_por_DCC', target='fin', conditions=[], on_error='rechazado_por_DCC', permission='')
    def rejected_by_DCC_to_end(self):
        # rechazado_por_DCC --> rechazado_por_DCC
        # send email to user that enviado the action
        print('The adaptation action is transitioning from <rechazado_por_DCC> to <fin>')
        ...
    
    @transition(field='fsm_state', source='cambios_solicitados_por_DCC', target='actualizando_por_solicitud_DCC', conditions=[can_update_by_DCC_request], on_error='cambios_solicitados_por_DCC', permission='')
    def update_by_DCC_request(self, user_approver):
        # cambios_solicitados_por_DCC --> actualizando_por_solicitud_DCC
        # send email to user that enviado the action
        print('The adaptation action is transitioning from <cambios_solicitados_por_DCC> to <actualizando_por_solicitud_DCC>')
    ...
    ## aceptado_por_DCC to	registrado_por_DCC
    @transition(field='fsm_state', source='aceptado_por_DCC', target='registrado_por_DCC', conditions=[], on_error='aceptado_por_DCC', permission='')
    def registered_by_DCC(self):
        # aceptado_por_DCC --> registrado_por_DCC
        # send email to user that enviado the action
        print('The adaptation action is transitioning from <aceptado_por_DCC> to <registrado_por_DCC>')
        ...
